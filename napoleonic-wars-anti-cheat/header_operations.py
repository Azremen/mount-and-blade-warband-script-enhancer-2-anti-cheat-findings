################################################################################
#                              header_operations expanded v.1.0.2 (unofficial) #
################################################################################
# TABLE OF CONTENTS
################################################################################
...
call_script             =    1  # (call_script, <script_id>, [<script_param>...]),
                                ...
                                # Fail code below if the script fails or <script_id> parameter is -1.
                                # Doesn't work with logical operators <neg> and <this_or_next>.
...
else_try                =    5  # (else_try),
                                ...
                                # Works also within try_for_agents and try_for_range. Each loop will execute code block after else_try if code block before failed.
                                # There can be multiple else_try in the loops.
...
try_for_agents          =   12  # (try_for_agents, <agent_no>[, <position_no>, <radius_fixed_point>]), 
                                ...
                                # The reference <agent_no> is active untill the next mission or scene is loaded. So don't check agent_is_active.
                                # It will always return true.
                                # Optional parameters allow to check agents only within specific radius around certain position but without increasing performance.
                                # Radius is fixed point value so you need to specify fixed_point_multiplier before using this operator.
                                # If fixed_point_multiplier is changed inside the loop it should be restored before the next iteration.
                                ...
                                # Radius can be used to break loop if assigned to 0 wich will just run a loop without body.
                                ...
                                # consume memory aproximately 40Kb and try_for_agents iterates dead agents too.  Even small number of agents can drop frames
                                # from 60 to 40-50. This is an issue on the  arena. There are no a lot of agents or props, but the frames are dropping.
                                # WSE+
...
try_for_prop_instances  =   16  # (try_for_prop_instances, <instance_no>, [<object_id>][, <object_type>]),
                                ...
                                # <object_id> refers to <scene_prop_id> or <item_kind_id>. The loop takes only ID into account. Use somt_ flags to make the difference.
...
################################################################################
  ...
  # Mathematical operations deal with numbers. Warband's Module System can only
  ...
  # When avoiding zero division error you add code (val_max, divisor, 1).
  # But if divisor is fixed_point than add (val_max, divisor, fixed_point_multiplier).
  # Otherwise the result of division will be multiplied by fixed_point_multiplier.
...
store_sqrt                 = 2125    # (store_sqrt, <destination_fixed_point>, <value_fixed_point>),
                                     # Assigns dest := SQRT (value). Result will be rounded up (to the biggest integer value).
...
store_atan2                = 2143    # (store_atan2, <destination_fixed_point>, <y_fixed_point>, <x_fixed_point>),
                                     # Returns the angle between the x axis and a point with coordinates (X,Y) in degrees. Note the angle is calculated counter-clockwise, i.e. (1,1) will return 45, not -45. Returns values in -180..180 range.
...
store_random_in_range      = 2136    # (store_random_in_range, <destination>, <range_low>, <range_high>),
                                     ...
                                     # The returned value is limited to the range of 32767 = 0x7FFF.
                                     # For example, (store_random_in_range, random, 10, 0xFFFF), will return values from minimum 10 to maximum (0x7FFF + 10).
                                     # If range_high <= range_low, assigns random to range_low value.
shuffle_range              = 2134    # (shuffle_range, <reg_no>, <upper_bound>),
...
store_trigger_param         = 2070  # (store_trigger_param, <destination>, <trigger_param_no>),
                                    # Version 1.153+. Retrieve the value of arbitrary trigger parameter. Parameters are enumerated starting from 1. Note that trigger can't have more than 3 parameters, no matter the number.
...
set_trigger_result          = 2075  # (set_trigger_result, <value>),
                                    ...
                                    # If the game script is called inside the trigger <set_trigger_result> sets result to both of them.
...
game_key_clicked                = 73  # (game_key_clicked, <game_key_code>),
                                      ...
                                      # Doesn't work for party window on global map.
...
map_free                   =   37  # (map_free),
                                   # Checks that the player is currently on the global map and can move freely, so no game screens are open, no pop-ups, player isn't resting, isn't in captivity of other party. Can be used with <neg> to fire triggers when he is involved in something (training peasants, collecting taxes, imprisoned, resting).

# Weather-handling operations

get_global_cloud_amount    =   90  # (get_global_cloud_amount, <destination>),
                                   # Returns current cloudiness (a value between 0..100). If value is 65 or higher, there will be rain in combat encounters.
set_global_cloud_amount    =   91  # (set_global_cloud_amount, <value>),
                                   # Sets current cloudiness (value is clamped to 0..100). If value is 65 or higher, there will be rain in combat encounters.
get_global_haze_amount     =   92  # (get_global_haze_amount, <destination>),
                                   # Returns current fogginess (value between 0..100). If value is 91 or higher, there will be fog in combat encounters.
set_global_haze_amount     =   93  # (set_global_haze_amount, <value>), 
                                   # Sets current fogginess (value is clamped to 0..100). If value is 91 or higher, there will be fog in combat encounters.
...
################################################################################
  ...
  # can be created at runtime. This makes parties ideal candidates for
...
spawn_around_party                    = 1100  # Ascension alternative (call_script, "script_cf_spawn_party", <party_id>, <party_template_id>),
                                              # Creates a new party from a party template and puts it's <party_id> into reg0.
                                              # vanila syntax (spawn_around_party, <party_id>, <party_template_id>),
...
party_collect_attachments_to_party    = 1662  # (party_collect_attachments_to_party, <source_party_id>, <aggregated_party_id>),
                                              # Creates an aggregated party from <source_party_id> and all parties attached to it.
                                              # Clears <aggregated_party_id> before adding troops and also adds wounded troops.
...
add_companion_party                   = 1233  # (add_companion_party, <troop_id_hero>),
                                              # Creates a new empty party at the exact player position with specified hero as party leader and the only member.
                                              # Party takes his name, uses pt_hero_party template and doesn't store its reference in reg0. It also gets pf_limit_members flag.
...
party_force_add_members               = 1613  # (party_force_add_members, <party_id>, <troop_id>, <number>),
                                              # Adds troops to party ignoring party size limits. Mostly used to add hero troops. Doesn't add if troop already exist in the party.
...
remove_troops_from_companions         = 1215  # (remove_troops_from_companions, <troop_id>, <value>),
                                              # Removes troops from player's party with message of removed troops number.
                                              # Unlike (party_remove_members) will not store number of removed in reg0.
                                              # Apparently bugged. In the message the name of the troop will be (null) and it will not show the number of troops removed
                                              # but the value of troops to remove passed as argument. But with npc it works fine.
...
party_get_free_prisoners_capacity     = 1634  # (party_get_free_prisoners_capacity, <destination>, <party_id>),
                                              ...
                                              # Apparently it will call <script_game_get_party_prisoner_limit> internally
                                              # and returned value for unlimited parties is wrong. Probably it is for <p_main_party>.
...
party_stack_get_size                  = 1653  # (party_stack_get_size, <destination>, <party_id>, <stack_no>),
                                              ...
                                              # WSE2: Can return 0 after battle and before returning on the global map.
...
store_num_regular_prisoners           = 2159  # (store_num_regular_prisoners, <destination>),
                                              # Stores number of regular prisoners in the player party.
...
store_main_party_wounded              = 2180  # (store_main_party_wounded, <destination>),
                                              # Calculates wounded regulars and NPCs of the main player party and stores into <destination>.
...
party_add_xp_to_stack                 = 1670  # (party_add_xp_to_stack, <party_id>, <stack_no>, <xp_amount>),
                                              ...
                                              # Calls <script_game_get_upgrade_xp> internally.
party_upgrade_with_xp                 = 1673  # (party_upgrade_with_xp, <party_id>, <xp_amount>, [<upgrade_path>]), #upgrade_path can be:
                                              # Awards specified number of xp points to entire party (split between all stacks) and upgrades all eligible troops. Upgrade direction: (0 = random (default), 1 = first, 2 = second).
...
party_get_battle_opponent             = 1680  # (party_get_battle_opponent, <destination>, <party_id>),
                                              ...
                                              # When the party is attached to another party this operator also returns opponent of the root party.
inflict_casualties_to_party_group     = 1697  # (inflict_casualties_to_party_group, <parent_party_id>, <damage_amount>, <party_id_to_add_causalties_to>),
                                              ...
                                              # Kills <damage_amount> units (members) randomly in the parent party and attached parties.
...
party_set_flags                       = 1603  # (party_set_flags, <party_id>, <flag>, <clear_or_set>),
                                              # Sets (1) or clears (0) the bits of party flags at runtime. See header_parties.py for the flag definitions and reference.
                                              # Capable of changing the party icon too, which is encoded as the lowest 8 bits of the flags (it's easier with party_set_icon though).
...
party_set_helpfulness                 = 1647  # (party_set_helpfulness, <party_id>, <number>),
                                              # Sets AI helpfulness value (tendency to help friendly parties under attack) for the party (range 0..10000, default 100).
...
add_xp_to_troop                          = 1062  # (add_xp_to_troop, <value>, [troop_id]),
                                                 # Adds xp points to troop. Default troop_id is player. Amount of xp can be only positive. Troop can be hero or regular.
...
troop_raise_proficiency                  = 1522  # (troop_raise_proficiency, <troop_id>, <proficiency_no>, <value>),
                                                 # Increases troop weapon proficiency by the specified value. Value can be negative. Increase is subject to limits defined by 
                                                 # Weapon Master skill. When used on non-hero troop, will affect all instances of that troop.
...
store_troop_health                       = 2175  # (store_troop_health, <destination>, <troop_id>, [absolute]),
                                                 # Returns the health percentage in the range (0-100). Set absolute to 1 to get the actual health in hit points.
troop_set_health                         = 1560  # (troop_set_health, <troop_id>, <value>, [absolute]),
                                                 # Sets troop health in percentage (0-100). Set absolute to 1 to set the actual health in hit points.
troop_get_upgrade_troop                  = 1561  # (troop_get_upgrade_troop, <destination>, <troop_id>, <upgrade_path>),
                                                 # Retrieves possible directions for non-hero troop upgrade. Use 0 to retrieve first upgrade path, and 1 to return second.
                                                 # Result of 0 means there's no such upgrade path for this troop.
...
troop_equip_items                        = 1533  # (troop_equip_items, <troop_id>),
                                                 # Makes the troop reconsider it's equipment. If troop has better stuff in it's inventory, he will equip it.
...
troop_loot_troop                         = 1539  # (troop_loot_troop, <target_troop>, <source_troop_id>, <probability>), 
                                                 ...
                                                 # Does not actually remove items from source_troop. Commonly used in Native to generate random loot after the battle.
                                                 # <itp_unique> items are ignored. The result is a random chance of adding some number of items. Repeat in cycle if needed.
...
troop_get_inventory_slot                 = 1541  # (troop_get_inventory_slot, <destination>, <troop_id>, <inventory_slot_no>),
                                                 ...
                                                 # Order is weapon1, weapon2, weapon3, weapon4, head_armor, body_armor, leg_armor, hand_armor, horse. Deprecated 9th slot was for food.
...
troop_add_gold                           = 1528  # (troop_add_gold, <troop_id>, <value>),
                                                 # Adds gold to troop. Generally used with player or hero troops. Can add negative values.
troop_remove_gold                        = 1529  # (troop_remove_gold, <troop_id>, <value>),
                                                 # Removes gold from troop. Generally used with player or hero troops. Can't subtract negative values.
...
# A handy web tool to inspect and tweak face codes [https://swyter.github.io/mab-tools/face can be found here].
# The face key internal format [https://forums.taleworlds.com/index.php?threads/online-face-code-generator-tool-tweak-facekeys-and-see-how-they-work.453145/#format],
# its various fields, sizes and bit layout is documented in the TaleWorlds forum by Swyter.
...
face_keys_get_hair                       = 2752  # (face_keys_get_hair, <destination>, <string_no>),
                                                 # Version 1.161+. Unpacks the current hair mesh index for the current race from a string containing troop/player face keys
                                                 # into <destination>.
face_keys_set_hair                       = 2753  # (face_keys_set_hair, <string_no>, <value>),
                                                 # Version 1.161+. Updates the face keys string with a new hair index for the current race. The first hair mesh starts
                                                 # from index 1. Use 0 for no hair. The field holds 6 bits; so it goes from 0 to 63.
...
face_keys_set_face_texture               = 2757  # (face_keys_set_face_texture, <string_no>, <value>),
                                                 # Version 1.161+. Updates face keys string with a new skin (originally <face_texture>) index value. Skins and their face textures
                                                 # are defined per-race (inside module_skins), and the first one begins at index 0. Range is 0-63.
face_keys_get_hair_texture               = 2758  # (face_keys_get_hair_texture, <destination>, <string_no>),
                                                 # Version 1.161+. Deprecated/old field, now depends on the active skin. Originally meant to unpack the active hair texture index
                                                 # from a string containing troop/player face keys into <destination>. Apparently hair textures have no effect.
                                                 # [https://discord.com/channels/411286129317249035/411291053702774784/1258158941058236478]
                                                 # - @Swyter: After some R&D, I think <hair_texture> and <skin_color> are leftovers from earlier versions, as now these fields are
                                                 # paired with the <face_texture/skin>. The fields are there and can be read and written, but I don't think they do anything.
                                                 # Updated my face code format doc here: [https://forum.taleworlds.com/?threads/453145/#format]
face_keys_set_hair_texture               = 2759  # (face_keys_set_hair_texture, <string_no>, <value>),
                                                 # Version 1.161+. Deprecated/old field, now depends on the active skin. See above. Updates face keys string with a new hair
                                                 # texture value. Doesn't seem to have an effect.
face_keys_get_hair_color                 = 2760  # (face_keys_get_hair_color, <destination>, <string_no>),
                                                 # Version 1.161+. Unpacks the hair color slider value from face keys string. Values are in the range of 0..63.
                                                 # Mapping to specific colors depends on the hair color range defined for currently selected skin / face_texture combination.
...
face_keys_get_age                        = 2762  # (face_keys_get_age, <destination>, <string_no>),
                                                 # Version 1.161+. Unpacks the age slider value from face keys string. Values are in the range of 0..63.
face_keys_set_age                        = 2763  # (face_keys_set_age, <string_no>, <value>),
                                                 # Version 1.161+. Updates the face keys string with a new age slider value. Value should be in the 0..63 range.
face_keys_get_skin_color                 = 2764  # (face_keys_get_skin_color, <destination>, <string_no>),
                                                 # Version 1.161+. Deprecated/old field. Retrieves the data from this unused field; it does work but nowadays it is not exposed
                                                 # in the editor, and instead the skin color is defined according to the currently active skin (i.e. the <face_texture> field).
                                                 # Originally meant for retrieving the skin color value from face keys string into <destination>. Goes from 0 to 63.
                                                 # You usually get a 0 unless manually set.
                                                 # [https://discord.com/channels/411286129317249035/411291053702774784/1258143494506020906]
                                                 # - @Swyter: I have been testing <face_keys_get_skin_color>/<face_keys_set_skin_color> and they do work, but the field
                                                 # doesn't seem to be exposed in the face editor, I don't know what it does, goes from 0 to 63:
                                                 # [https://gist.github.com/Swyter/0b03cdae54886afc5ff2e51f3d8f1223?permalink_comment_id=5110111#gistcomment-5110111]
face_keys_set_skin_color                 = 2765  # (face_keys_set_skin_color, <string_no>, <value>),
                                                 # Version 1.161+. Deprecated/old field. Does work but the field is unused and a leftover in modern game versions, see above.
                                                 # Can potentially be used to store custom stuff. Originally meant to update face keys string with a new skin color value.
                                                 # Goes from 0 to 63.
face_keys_get_morph_key                  = 2766  # (face_keys_get_morph_key, <destination>, <string_no>, <key_no>),
                                                 # Version 1.161+.  Unpacks a morph key (i.e. face shape adjustment slider) value from a hexadecimal face keys string.
                                                 # See your race-dependent morph keys and their order in your mod's module_skins.py file; they appear listed from bottom to top
                                                 # in the face editor GUI. First element starts at zero.
# Note that this operation is broken and only the first 8 sliders/indices (from <key_no> index 0 to 7) out of a theoretical maximum of 0-41 morph key sliders are actually
# accessible (from '<chin_size>' to '<cheeks>' in Native).
# Each provided morph key value has three bits, so the slider value always fits into the 0..7 range.
# ''There is a replacement <face_keys_get_extended_morph_key> function that removes most of the key_no limits here.''
# https://forums.taleworlds.com/index.php?threads/face-code-module-system-trickery-scripts-to-read-more-face-attributes-measuring-string-lengths-and-doing-string-to-number-conversion.453646/#post-9823956

face_keys_set_morph_key                  = 2767  # (face_keys_set_morph_key, <string_no>, <key_no>, <value>),
                                                 # Version 1.161+. Updates a face keys string with a new morph key (i.e. face shape adjustment slider) value.
                                                 # See your race-dependent morph keys and their order in your mod's module_skins.py file; they appear listed from bottom to top
                                                 # in the face editor GUI. First element starts at zero.
#  Note that this operation is broken and only the first eight (from <key_no> index 0 to 7) morph key sliders are actually updated (from '<chin_size>' to '<cheeks>' in Native).
# The rest are artificially ignored due to a bug 
# https://discord.com/channels/411286129317249035/411291053702774784/1258349857203163166
# - @K700: v1399 < 0 || ((_QWORD)&v1399 + 1) >= 8i64 bugged check, should be 42.
# Each provided morph key value has three bits, so the slider value always fits into the 0..7 range. 
# ''There is a replacement <face_keys_set_extended_morph_key> function that removes all the key_no limits here.''
# https://forums.taleworlds.com/index.php?threads/python-script-scheme-exchange.8652/page-40#post-9898525
...
start_quest                   = 1280  # (start_quest, <quest_id>, <giver_troop_id>),
                                      ...
                                      # If <giver_troop_id> >= 0 it will be set as quest giver.
                                      # If set <giver_troop_id> := -1 conversation partner troop_ID is assigned as giver. If no conversation partner than it will assign -1 probably.
conclude_quest                = 1286  # (conclude_quest, <quest_id>),
                                      # Sets quest status as concluded, but at this point it generally remains in the list.
                                      # Frequently used to indicate "uncertain" quest status, when it's neither fully successful nor a total failure.
...
# leaving the <active> and any other existing flags (like <concluded>/<failed>/<succeeded>) toggled on and intact, the only way to reset the state is
# via the <cancel_quest = 1284> operation.
...
set_quest_progression         = 1285  # (set_quest_progression, <quest_id>, <value>),
                                      # Deprecated and useless, the operation has no game effects and it's impossible to retrieve quest progression status anyway.
...
item_has_modifier                   = 2725  # (item_has_modifier, <item_kind_no>, <item_modifier_no>),
                                            ...
                                            # Vanilla Warband has x32 limit so any modifier hire than 31 is not shown. Strange behavior will show <imod_swaybacked = 31> if item has
                                            # modifiers hire than 31. WSE2 doesn't have x32 limit bug.
item_has_faction                    = 2726  # (item_has_faction, <item_kind_no>, <faction_no>),
                                            # Version 1.161+. Checks that the item is available for specified faction. Note that an item with no factions set is available to
                                            # all factions despite this operation returning false for each faction.
...
store_last_sound_channel =  615  # (store_last_sound_channel, <destination>),
                                 ...
                                 # Sounds with <sf_stream_from_hd> flag will not occupy channel, i.e. operand return -1.
...
position_copy_origin                        =  719  # (position_copy_origin, <position_target>, <position_source>),
                                                    # Copies coordinates from source position to target position, without changing rotation nor scale.
position_copy_rotation                      =  718  # (position_copy_rotation, <position_target>, <position_source>),
                                                    # Copies rotation and scale from source position to target position, without changing coordinates.
...
position_get_rotation_around_x              =  742  # (position_get_rotation_around_x, <destination>, <position>),
                                                    # Returns angle (in degrees rounded to nearest integer) that the position is rotated around X axis (tilt forward/backwards). 
position_get_rotation_around_y              =  743  # (position_get_rotation_around_y, <destination>, <position>),
                                                    # Returns angle (in degrees rounded to nearest integer) that the position is rotated around Y axis (tilt right/left).
position_get_rotation_around_z              =  740  # (position_get_rotation_around_z, <destination>, <position>),
                                                    # Returns angle (in degrees rounded to nearest integer) that the position is rotated around Z axis (turning right/left).
...
position_set_scale_x                        =  744  # (position_set_scale_x, <position>, <value_fixed_point>),
                                                    # Sets position scaling along X axis. Changes position rotation.
position_set_scale_y                        =  745  # (position_set_scale_y, <position>, <value_fixed_point>),
                                                    # Sets position scaling along Y axis. Changes position rotation.
position_set_scale_z                        =  746  # (position_set_scale_z, <position>, <value_fixed_point>),
                                                    # Sets position scaling along Z axis. Changes position rotation.
...
get_angle_between_positions                 =  705  # (get_angle_between_positions, <destination_fixed_point>, <position_no_1>, <position_no_2>),
                                                    # Calculates angle between positions, using positions as vectors. Only rotation around Z axis is used. In other words, the function 
                                                    # returns the difference between Z rotations of both positions.
...
position_is_behind_position                 =  714  # (position_is_behind_position, <position_base>, <position_to_check>),
                                                    ...
                                                    # Bugged: fails to calculate for all axis rotated position or if there is slight deviation from axis.
...
map_get_land_position_around_position       = 1628  # (map_get_land_position_around_position, <dest_position_no>, <source_position_no>, <radius>),
                                                    # Returns a random position on the global map in the vicinity of the source_position. Will return a land position that can be
                                                    # walked on (which means any terrain type except rt_mountain, rt_mountain_forest, rt_water, rt_river, rt_bridge and
                                                    # rt_deep_water). Can fail code below if position wasn't founded.
map_get_water_position_around_position      = 1629  # (map_get_water_position_around_position, <dest_position_no>, <source_position_no>, <radius>),
                                                    # Returns a random position on the global map in the vicinity of the source_position. Will return a water position (which 
                                                    # means only rt_water, rt_river or rt_bridge - not even rt_deep_water, which - while looks like water - is considered land for
                                                    # the purpose of this operation). Can fail code below if position wasn't founded.
...
cur_agent_set_banner_tableau_material            = 1986  # (cur_agent_set_banner_tableau_material, <tableau_material_id>, <banner_mesh>),
...
cur_tableau_add_mesh_with_vertex_color           = 1993  # (cur_tableau_add_mesh_with_vertex_color, <mesh_id>, <position>, <scale_fixed_point>, <alpha_fixed_point>, <color_argb>),
...
str_store_string                = 2320  # (str_store_string, <string_register>, <string>),
                                        # Second parameter can be string_ID, quick_string, string_register or even a variable.
                                        # The variable value points to the string_ID.
str_store_string_reg            = 2321  # (str_store_string_reg, <string_register>, <string>),
                                        # Used for randomisation of quick_strings in dialogs.
                                        # Second parameter can be variable or string_register.
                                        # The variable value points to the string_register.
...
tutorial_message                    = 1122  # (tutorial_message, <string_id>, [color], [auto_close_time]),
                                            # Displays a popup window with tutorial text stored in referenced string or string register. Use -1 to close any currently open tutorial box. Optional parameters allow you to define text color (must include alpha bits) and time period after which the tutorial box will close automatically. Works also at the world map.
...
set_tooltip_text                      = 1130  # (set_tooltip_text, <string_id>),
                                              # Displays tooltips for menu options in the menu window. Erundil confirms that it works when the operation is called in the condition block of the menu-option entry
...
set_encountered_party                 = 2205  # (set_encountered_party, <party_no>),
                                              ...
                                              # Used in Native during chargen to set up the starting town and then immediately throw the player
                                              # into street fight without showing him the town menu.
...
set_visitor                                  = 1263  # (set_visitor, <entry_no>, <troop_id>, [<dna>]),
                                                     # Adds the specified troop as the visitor to the entry point of the scene defined with (modify_visitors_at_site).
                                                     # Entry point must have mtef_visitor_source type. Optional DNA parameter allows for randomization of non-hero agent looks.
...
set_visitors                                 = 1264  # (set_visitors, <entry_no>, <troop_id>, <number_of_troops>),
                                                     # Same as (set_visitor = 1263), but spawns an entire group of some troop type.
add_visitors_to_current_scene                = 1265  # (add_visitors_to_current_scene, <entry_no>, <troop_id>, <number_of_troops>[, <team_no>, <group_no>]),
                                                     # Adds a number of troops to the specified entry point when the scene is already loaded. Optional <team_no> and <group_no> parameters are used 
                                                     # in multiplayer mode only, singleplayer mode uses selected entry point settings.
                                                     # Default team is 7 wich can be overridden by <entry_no> flags. 7 team is hardcoded neutral for everyone and this couldn't be changed.
...
set_zoom_amount                              = 2221  # (set_zoom_amount, <value_fixed_point>),
                                                     # Version 1.153+. Sets new zoom rate. 100 is default, 197 is max. 198+ values will overflow and not recommended.
...
mission_cam_set_mode                         = 2001  # (mission_cam_set_mode, <mission_cam_mode>, [<duration-in-1/1000-seconds>], [<value>]),
                                                     # Not documented. Changes main camera mode. Camera mode is 0 for automatic and 1 for manual (controlled by code). Duration parameter is used when switching from manual to auto, to determine how long will camera move to it's new position. If value = 0, then camera velocity will be linear. Else it will be non-linear.
...
cast_ray                                     = 1900  # (cast_ray, <collision_instance>, <hit_position>, <start_position>, [<ray_length_fixed_point>]),
                                                     # Version 1.161+. Casts a ray from <start_position> and stores the <hit_position>.
                                                     # Only checks collisions with collision bodies, scene props with collision, and terrain.
                                                     # If the body hit is a scene prop, its instance id will be stored into <collision_instance>.
                                                     # Collision with terrain and trees returns -1.
                                                     # Can fail code below if no collision is made.
                                                     # Collision face's normals are stored in <hit_position> rotation with Y+ being the outward facing.
                                                     # Doesn't collide with agents. Longer rays are computationally more expensive.
set_postfx                                   = 2386  # (set_postfx, <postfx_params-ID>)
                                                     ...
                                                     # It can also be used on the world map, but (unlike during a mission) it resets every time a menu is visited,
                                                     # requiring setting postfx in every frame in case player opens something.
...
# The following operations let us define custom shader uniforms (i.e. global shader variables), named after the string in <parameter_name>;
# this very powerful functionality can be used to relay dynamic data from the module system to the GPU that will take effect the next time the screen is redrawn.

# So the only limit is that we can only refresh and update the contents of any of these uniforms per-frame (e.g. not per troop, they are called uniforms
# because the same data applies scene-wide). We can define as many uniforms as we want and they work both in missions, strategic mode, menus and loading screens.
# Each uniform survives across saves and modes, retaining its value unless updated or exiting the game.

# Any vertex or pixel shaders can use these uniforms to change/vary their behavior (think anything from wind amount, direction, ambient tinting, extra light points,
# time of day/night) dynamically. For a quick example that conditionally hides the HUD see [https://github.com/tldmod/tldmod/blob/849db8d70/_wb/mb_src.fx#L22-L23 here],
...
prop_instance_intersects_with_prop_instance = 1880  # (prop_instance_intersects_with_prop_instance, <checked_scene_prop_id>, <scene_prop_id>),
                                                    # Checks if two scene props are intersecting (i.e. collided). Useful when animating scene props movement. Pass -1 for second parameter to check the prop against all other props on the scene. Cannot check polygon-to-polygon physics models, but can check any other combinations between sphere, capsule and polygon physics models.
...
scene_prop_get_instance                     = 1811  # (scene_prop_get_instance, <destination>, <scene_prop_id>, <instance_no>),
                                                    # Retrieves the reference to a scene prop instance by its number. Fails if an instance of such index doesn't exist.
...
spawn_scene_prop                            = 1974  # (spawn_scene_prop, <scene_prop_id>),
                                                    # Spawns a new scene prop instance of the specified type at the position defined by the last call to (set_spawn_position) and saves the reference to the new prop in reg0.
...
scene_prop_set_visibility                   = 1813  # (scene_prop_set_visibility, <scene_prop_id>, <value>),
                                                    # Shows (value = 1) or hides (value = 0) the scene prop instance. Collision remains on invisible props.
...
prop_instance_enable_physics                = 1864  # (prop_instance_enable_physics, <scene_prop_id>, <value>),
                                                    # Enables (value = 1) or disables (value = 0) physics calculation (gravity, collision checks) for the scene prop instance. Disables the use prompt for usable objects as well. Note that even with (value = 0) it will still be detected by cast_ray and will still fail line of sight checks.
prop_instance_initialize_rotation_angles    = 1866  # (prop_instance_initialize_rotation_angles, <scene_prop_id>),
                                                    ...
                                                    # This operator resets rotation angles to zeroes.
prop_instance_rotate_to_position            = 1865  # (prop_instance_rotate_to_position, <scene_prop_id>, <position>, <duration-in-1/100-seconds>, <angle-in-1/100-degrees>),
                                                    # Specified prop instance will move to the target position within the specified duration of time, and within the same time
                                                    # it will rotate for the specified angle. Used in Native code to simulate behavior of belfry wheels and rotating winches.
                                                    # Can be stopped with <prop_instance_stop_animating = 1861>.
...
spawn_item_without_refill                   = 1976  # (spawn_item_without_refill, <item_kind_id>, [<item_modifier>], [seconds_before_pruning]),
                                                    # Version 1.153+. Napoleonic Wars. Spawns firearm without loaded bullet.
...
agent_is_routed                          = 1699  # (agent_is_routed, <agent_no>),
                                                 # Checks that the agent has fled from the map (i.e. reached the edge of the map in fleeing mode and then faded). Will return false for agent_fade_out though.
...
agent_is_active                          = 1712  # (agent_is_active, <agent_no>),
                                                 # Checks that the <agent_no> reference is active i.e. agent is valid. This will always succeed inside try_for_agents loop.
...
add_reinforcements_to_entry              = 1930  # (add_reinforcements_to_entry, <mission_template_entry_no>, <wave_size>),
                                                 # For battle missions, adds reinforcement wave to the specified entry line. Additional parameter determines relative wave size. Agents in reinforcement wave are taken from all parties of the side that the entry line belongs to due to mtef_team_* flags.
                                                 # If entry line has <mtef_use_exact_number> flag than wave size will be exact number.
set_spawn_position                       = 1970  # (set_spawn_position, <position>), ## DUPLICATE ENTRY
                                                 # Defines the position which will later be used by (spawn_scene_prop), (spawn_scene_item), (spawn_agent) and (spawn_horse) operations.
...
agent_get_position                       = 1710  # (agent_get_position, <position>, <agent_id>),
                                                 ...
                                                 # Footman agent skin scale is stored to Z-scale. Other X and Y scales are depending on Z rotation and are not constants.
                                                 # If position is moved locally the distance is multiplied by agent scale. Horse and rider can have different scales in the same position.
                                                 # The Z-scale of horseman is not constant because Y-axis can rotate.
...
agent_get_entry_no                       = 1717  # (agent_get_entry_no, <destination>, <agent_id>),
                                                 # Retrieves the entry where this agent has spawned. This entry is the ordered number of line at the first code block of mission template.
...
agent_set_hit_points                     = 1721  # (agent_set_hit_points, <agent_id>, <value>, [absolute]),
                                                 ...
                                                 # Value is truncated at the agent maximum hit points.
agent_set_max_hit_points                 = 2090  # (agent_set_max_hit_points, <agent_id>, <value>, [absolute]),
                                                 # Version 1.153+. Changes agent's max hit points and sets HP to this value. Optional flag [absolute] determines if <value> is an absolute number of his points, or relative percentage (0..1000) of default value. Treated as percentage by default.
                                                 # The relative value uses the current maximum health as the reference value for 100%, rather than the troop's original maximum health.
                                                 # Troop health also increases and matches agent health after battle ends and it returns to normal on the next scene (mission) and when moving around the global map.
...
# Shields are processed unnatural way. They are wielded at the agent spawn even if the primary item can't be used with.
...
# I assume this mechanic is suited for player only cause he can put a shield behind his back. This will trigger <ti_on_item_unwielded>.
# After unwielding it will not be wielded again with suitable items untill player wield it again with mouth scroll button that triggers <ti_on_item_wielded>.
# Shields don't brake from friendly fire shots. Braking shield triggers <ti_on_item_unwielded>. Shield item will be removed after trigger.
# Operation <(agent_set_wielded_item, agent, -1)> will unwield shield and any wielded item and trigger <ti_on_item_unwielded> for them.
# Operation <(agent_set_wielded_item, agent, item)> will trigger <ti_on_item_unwielded> and <ti_on_item_wielded> in ordered way.
# Equip/unequip agent operations don't trigger <ti_on_item_unwielded> and <ti_on_item_wielded>.
# You can think that shield is always wielded even if it is not in the arm.
# When shield is in the hand and agent switch to two-handed weapon (or not wieldable with shield) the shield is still in the hand in <ti_on_item_wielded>. Shield removed after trigger without
# triggering <ti_on_item_unwielded>.
# When shield isn't in the hand and agent switch to one-handed weapon (or wieldable with shield) the shield is not in the hand in <ti_on_item_wielded>. Shield added after trigger if it
# was wielded previously (has triggered <ti_on_item_wielded>).
# Dropping two-handed (not wieldable with shield) weapon on the ground will not trigger <ti_on_item_unwielded> for dropped weapon. But puts shield in the hand if it was wielded previously.
# Picking up two-handed (not wieldable with shield) weapon on the ground will trigger <ti_on_item_wielded> for picked item. Shield removed after trigger.
# So to cover dropping weapons you should use with <ti_on_item_dropped>.
...
agent_get_wielded_item                   = 1726  # (agent_get_wielded_item, <destination>, <agent_id>, <is_secondary>),
                                                 # Retrieves the item reference that the agent is currently wielding.
                                                 # Set <is_secondary> to 1 for shields and 0 for primary items.
agent_get_ammo                           = 1727  # (agent_get_ammo, <destination>, <agent_id>, [<value>]),
                                                 ...
                                                 # Doesn't count ammo loaded in crossbow or firearm.
                                                 # If <value> is set to 1 and there is no wielding item returns -1.
agent_get_item_cur_ammo                  = 1977  # (agent_get_item_cur_ammo, <destination>, <agent_id>, <slot_no>),
                                                 # Version 1.153+. Napoleonic Wars. Returns remaining ammo for specified agent's item, i.e. how much bullets loaded.
agent_refill_ammo                        = 1728  # (agent_refill_ammo, <agent_id>),
                                                 ...
                                                 # Doesn't refill weapon slot for crossbow and firearm. Operand <agent_set_ammo> can.
agent_set_wielded_item                   = 1747  # (agent_set_wielded_item, <agent_id>, <item_id>),
                                                 # Forces the agent to wield the specified item. Agent must have that item in his equipment for this to work.
                                                 # Use item_id = -1 to unwield any currently wielded item.
                                                 # Agent wielding item is alarmed and can't be unalarmed. The animation changes to alarmed that has a hire priority.
...
agent_set_no_dynamics                    = 1762  # (agent_set_no_dynamics, <agent_id>, <value>),
                                                 ...
                                                 # When set from 1 to 0 mid-air on non-player agent, will leave it floating if the ground below is more than ~2m away. Player will fall no matter how far ground is. 
...
agent_set_attached_scene_prop            = 1757  # (agent_set_attached_scene_prop, <agent_id>, <scene_prop_id>),
                                                 # Attaches the specified prop instance to the agent. Used in multiplayer CTF missions to attach flags to players. The scale factor at the skin entry gets applied to the scene prop.
...
agent_set_defend_action                  = 1746  # (agent_set_defend_action, <agent_id>, <value>, [<duration-in-1/1000-seconds>]),
                                                 ...
                                                 # The operation will be ignored entirely if duration is less then 0 or the <action_value> does not match the allowed actions.
...
agent_ai_set_always_attack_in_melee      = 1737  # (agent_ai_set_always_attack_in_melee, <agent_id>, <value>),
                                                 # If you hide behind some obstacle diappearing from line of sight of enemy archers, they stand there and wait until you show up again
                                                 # or making a step or two in your direction occasionally. This flag makes them switch to melee instantly and charge you without waiting.
...
################################################################################
  ...
  # Buttons are clickable objects. Button is clickable even if not visible.
  # It can be hidden by alpha channel (transparency) or other overlays (meshes or buttons with highest overlay_ID or render height).
  # If two buttons occupy the same area the one with highest overlay_ID will trigger.
...
create_game_button_overlay                        =  940  # (create_game_button_overlay, <destination>, <string_id>),
                                                          # Creates a game button overlay and returns it's overlay_id. The size specified with <overlay_set_size>.
...
overlay_set_boundaries                            =  928  # (overlay_set_boundaries, <overlay_id>, <min_value>, <max_value>),
                                                          ...
                                                          # Doesn't work for number_box.
overlay_set_position                              =  926  # (overlay_set_position, <overlay_id>, <position>),
                                                          # Sets the overlay position on the screen, using position's X and Y coordinates.
...
overlay_set_size                                  =  925  # (overlay_set_size, <overlay_id>, <position>),
                                                          # Sets the overlay size, using position's X and Y coordinates. Doesn't work on numberbox overlays. Note that the screen size in Warband is (1.00,0.75), further modified by fixed point multiplier. Also see (overlay_set_area_size).
...
send_message_to_url                          =  380  # (send_message_to_url, <string_id>, <encode_url>),
                                                     # Accesses the the URL in the provided string and sends an HTTP request. The result will be returned to "script_game_receive_url_response". Parameter <encode_url> is optional and effects are unclear. Supposedly it's equivalent of calling (str_encode_url) on the first parameter which doesn't make sense for me.
...
server_set_combat_speed                      =  479  # (server_set_combat_speed, <value>),
                                                     # Official docs: 0-2
                                                                             # Unofficial: <value> is actually 0-4, going from slowest to fastest, you can see it's used in module_presentations from 0 to 4                                                                                                                            
...
# str_store_attribute_name        # (str_store_attribute_name, <string_reg_no>, <attribute_id>),
                                  # Stores specified attribute name (3 capitalized letters by default) to string register, as specified in module_ui_strings.
# str_store_attribute_name_full   # (str_store_attribute_name_full, <string_reg_no>, <attribute_id>),
                                  # Stores specified attribute full name to string register, as specified in module_ui_strings.
# str_store_proficiency_name      # (str_store_proficiency_name, <string_reg_no>, <proficiency_id>),
                                  # Stores specified proficiency name to string register.
# str_store_skill_name            # (str_store_skill_name, <string_reg_no>, <skill_id>),
                                  # Stores specified skill name (as specified in module_skills) to string register
# str_store_item_modifier_name    # (str_store_item_modifier_name, <string_reg_no>, <imod_value>),
                                  # Stores specified item modifier name (as specified in module_item_modifiers) to string register.
# str_store_item_id_repr          # (str_store_item_id_repr, <string_reg_no>, <item_id>),
                                  # Stores specified item id name (as specified in module_items) to string register.
# str_store_damage_type_name      # (str_store_damage_type_name, <string_reg_no>, <damage_type>),
                                  # Stores specified damage type name to string register.
# str_store_damage_type_adjective # (str_store_damage_type_adjective, <string_reg_no>, <damage_type>),
...
# push_value                 # (push_value, <value>),
                             # Saves a single value to stack.
# pop_value                  # (pop_value, <destination>),
                             # Retrieves a single value from top of stack.
# peek_value                 # (peek_value, <destination>),
...
# start_mission_timer        # (start_mission_timer, <timer_index>),
                             # Starts a new mission timer with specified index.
# store_mission_timer        # (store_mission_timer, <destination>, <timer_index>),
                             # Stores current value (in milliseconds) for specified mission timer.
# store_script_params        # (store_script_params, <param1>, <param2>...),
                             # Retrieves arbitrary number of script parameters with a single line.
# get_fixed_point_multiplier # (get_fixed_point_multiplier, <destination>),
                             # Retrieves current fixed point multiplier value.
# position_set_coordinates   # (position_set_coordinates, <pos>, <x_fixed_point>, <y_fixed_point>, <z_fixed_point>),
                             # Sets all 3 position coordinates with a single line.
# position_aim_at_position   # (position_point_towards, <position_reg>, <aim_position_reg>),
                             # Rotates the position so it's Y axis points at specified aim position.
# troop_set_attribute        # (troop_set_attribute, <troop_id>, <attribute_id>, <value>),
                             # Sets troop attribute to specified value.
# troop_set_skill            # (troop_set_skill, <troop_id>, <skill_id>, <value>),
                             # Sets troop skill to specified value.
# troop_is_not_capturable_alive # (troop_is_not_capturable_alive, <troop_id>),
                             # Check if troop is not capturable alive.
# troop_is_allways_fall_dead # (troop_is_allways_fall_dead, <troop_id>),
                             # Check if troop is allways fall dead.
# troop_is_unkillable        # (troop_is_unkillable, <troop_id>),
                             # Check if troop is unkillable.

# item_get_real_difficulty   # (item_get_real_difficulty, <destination>, <item_id>, [<imod_value>]),
                             # Retrieves item difficulty value, taking modifier into account. If modifier is omitted, standard item difficulty is returned.

# item_modifier_get_damage            # (item_modifier_get_damage, <destination>, <imod_value>),
# item_modifier_get_armor             # (item_modifier_get_armor, <destination>, <imod_value>),
# item_modifier_get_hp                # (item_modifier_get_hp, <destination>, <imod_value>),
# item_modifier_get_weapon_speed      # (item_modifier_get_weapon_speed, <destination>, <imod_value>),
# item_modifier_get_difficulty        # (item_modifier_get_difficulty, <destination>, <imod_value>),
# item_modifier_get_horse_charge      # (item_modifier_get_horse_charge, <destination>, <imod_value>),
# item_modifier_get_horse_speed       # (item_modifier_get_horse_speed, <destination>, <imod_value>),
# item_modifier_get_horse_maneuver    # (item_modifier_get_horse_maneuver, <destination>, <imod_value>),
# item_modifier_get_rarity_multiplier # (item_modifier_get_rarity_multiplier, <destination_fixed_point>, <imod_value>),
# item_modifier_get_value_multiplier  # (item_modifier_get_value_multiplier, <destination_fixed_point>, <imod_value>),

# try_for_party_group       # (try_for_party_group, <destination>, <source_party>),
                            # Loops for all parties in the group including source party.
# val_divup                 # (val_divup, <destination>, <value>),
                            # Divide destination with round up. Only positive values.
# store_divup               # (store_divup, <destination>, <value1>, <value2>),
                            # Divide destination := value1/value2 with round up. Only positive values.
# get_animation_duration    # (get_animation_duration, <destination>, <animation_id>),
                            # Get animation duration in milliseconds. Only minimal duration of different variants is counted.

# ui_create_label           # (ui_create_label, <destination>, <string>, <x>, <y>, [<alignment>], [<scale>], [<rotation>]),
                            # See header_presentations.py file for text alignment constants.
# ui_create_mesh            # (ui_create_mesh, <destination>, <mesh>, <x>, <y>, [<scale_x>], [<scale_y>]),
                            # If only <scale_x> is provided, it will be used for both dimensions.
# ui_create_button          # (ui_create_button, <destination>, <string>, <x>, <y>, [<scale_x>], [<scale_y>]),
                            # If only <scale_x> is provided, it will be used for both dimensions.
# ui_create_game_button     # (ui_create_game_button, <destination>, <string>, <x>, <y>, [<scale_x>], [<scale_y>]),
                            # If only <scale_x> is provided, it will be used for both dimensions.
# ui_create_in_game_button  # (ui_create_in_game_button, <destination>, <string>, <x>, <y>, [<scale_x>], [<scale_y>]),
                            # If only <scale_x> is provided, it will be used for both dimensions.
# ui_create_image_button    # (ui_create_image_button, <destination>, <mesh_base>, <mesh_clicked>, <x>, <y>, [<scale_x>], [<scale_y>]),
                            # If only <scale_x> is provided, it will be used for both dimensions.
# ui_create_checkbox        # (ui_create_checkbox, <destination>, <mesh_off_state>, <mesh_on_state>, <x>, <y>, [<start_value>], [<scale>]),
                            # Typically "mesh_checkbox_off", "mesh_checkbox_on" are used as checkbox meshes. Checkbox is not checked by default.
# ui_create_container       # (ui_create_container, <destination>, <x>, <y>, <width>, <height>),
                            # Creates a scrollable area with specified dimensions.
# ui_create_numberbox       # (ui_create_numberbox, <destination>, <x>, <y>, <min_value>, <max_value>, [<scale_x>], [<scale_y>]),
                            # Normal empty numberbox.
# ui_create_textbox         # (ui_create_textbox, <destination>, <x>, <y>, [<scale_x>], [<scale_y>]),
                            # Normal empty textbox.
# ui_create_combobox        # (ui_create_combobox, <destination>, <x>, <y>, [<scale_x>], [<scale_y>], [<list_item>,...]),
                            # You can add any number of combobox items in this operation instead of using (overlay_add_item).

# ui_create_combobox_scripted    # (ui_create_combobox_scripted, <destination>, <x>, <y>, <item_names_script>, <num_items>, <value>, [<scale_x], [<scale_y>])
...
# overlay_scripted_set_display   # (overlay_scripted_set_display, <overlay>, <display>),
# overlay_scripted_set_val       # (overlay_scripted_set_val, <overlay>, <value>),
# overlay_scripted_get_val       # (overlay_scripted_get_val, <destination>, <overlay>),
# overlay_scripted_item_set_text # (overlay_scripted_item_set_text, <overlay>, <item>, <text>), items are indexed from 0.

# ui_create_listbox         # (ui_create_listbox, <destination>, <x>, <y>, [<scale_x>], [<scale_y>], [<list_item>,...]),
                            # You can add any number of combobox items in this operation instead of using (overlay_add_item).                                      

# ui_create_item            # (ui_create_item, <destination>, <item>, <x>, <y>, [<scale_x>], [<scale_y>]),
                            # If only <scale_x> is provided, it will be used for both dimensions.
# ui_create_horslider       # (ui_create_horslider, <destination>, <x>, <y>, <min_value>, <max_value>, [<scale_x>], [<scale_y>]),
                            # Creates horisontal slider.
# ui_create_mesh_with_tableau_material # (ui_create_mesh_with_tableau_material, <destination>, <mesh>, <tableau_material>, <value>, <x>, <y>, [<scale_x>], [<scale_y>]),
...
# WSE code will be skipped in Vanilla
# (try_begin),
#   (neg|is_vanilla_warband),
#   (. . .), # code for WSE
# (try_end),

# WSE alternative code
# (try_begin),
#   (is_vanilla_warband), # fails when WSE is running
#   (. . .), # code for vanilla
# (else_try),
#   (. . .), # code for WSE
# (try_end),      
...
start_map_conversation       = 1025 #(start_map_conversation, <troop_id>, [troop_dna], [set_dialog_state], [dialog_state], [from_presentation]), #Starts a conversation with the selected <troop_id>. Can be called directly from global map or game menus. [<troop_dna>] parameter allows you to randomize non-hero troop appearances. If [<set_dialog_state>] sets, then [<dialog_state>] used instead dlg_event_triggered. [<from_presentation>] - can be started from presentation and returns to that presentation after dialog close.
...
agent_set_can_multihit                           = 3353 #(agent_set_can_multihit, <agent_no>, <value>, [<max_targets>]), #Enables or disables <agent_no>'s  melee attacks to hit multiple enemies in a single swing, applying damage to every valid target intersecting the weapon's hitbox. Each target is hit at most once per swing. Attacks can still be blocked. If the agent is using a crush-through-block weapon, the swing will bypass blocks under the normal crush-through rules and continue past blocked defenders. Optional [<max_targets>] caps the number of distinct agents that can be damaged in a single swing (0 = unlimited, default). Must be set on the server side in multiplayer. (requires WSE2)
agent_has_multihit                               = 3354 #(agent_has_multihit, <agent_no>), #Fails if the <agent_no> has multi-hit disabled. (requires WSE2)
...
agent_set_can_piercing                           = 3359 #(agent_set_can_piercing, <agent_no>, <value>, [<max_targets>]), #Enables or disables <agent_no>'s ranged projectiles (arrows, bolts, thrown weapons) to pierce through enemies instead of stopping on the first hit. Each agent is hit at most once per projectile. Shield blocks, friendly hits, scene geometry and mission objects still stop the projectile. Optional [<max_targets>] caps the number of distinct agents (including horses) that a single projectile can damage (0 = unlimited, default). Must be set on the server side in multiplayer. (requires WSE2)
agent_has_piercing                               = 3360 #(agent_has_piercing, <agent_no>), #Fails if the <agent_no> has piercing projectiles disabled. (requires WSE2)
...
prop_instance_get_sound_progress     = 3618 #(prop_instance_get_sound_progress, <destination>, <prop_instance_no>), #Stores <prop_instance_no>'s sound_progress into <destination>. Returned value can be between 0-100, or -1 if nothing is being played. (requires WSE2)
set_horse_friendly_fire              = 3619 #(set_horse_friendly_fire, <value>), #Enables or disables horse friendly fire for singleplayer
cast_ray_agents                      = 3620 #(cast_ray_agents, <destination>, <hit_position_register>, <ray_position_register>, [<ray_length_fixed_point>]),
                                            #Casts a ray of length [<ray_length_fixed_point>] starting from <ray_position_register> and stores the closest agent's hit position into <hit_position_register> (fails if no hits).
...
# WSE2 addon operations
player_get_anticheat_stat     = 2908 #(player_get_anticheat_stat, <destination>, <player_no>, <stat_no>), #Stores anti-cheat statistic acs_* of <player_no> into <destination>. Server only. Values marked (fp) are fixed point (requires WSE2)
player_anticheat_report       = 2909 #(player_anticheat_report, <player_no>, <value>), #Reports a module-side detection for <player_no> through the anti-cheat pipeline (log, ti_on_cheat_detected, kick). Server only (requires WSE2)
server_set_anticheat_option   = 3519 #(server_set_anticheat_option, <option_no>, <value>), #Sets anti-cheat option aco_* to <value> at runtime. Server only. Float options take fixed point values (requires WSE2)
server_get_anticheat_option   = 3520 #(server_get_anticheat_option, <destination>, <option_no>), #Stores anti-cheat option aco_* into <destination>. Float options are returned as fixed point (requires WSE2)
...
edit_mode_window_open         = 4609 #(edit_mode_window_open), #Fails if the edit mode window is not open
skeleton_model_set_bone_body_section = 5201 #(skeleton_model_set_bone_body_section, <skeleton_model_name>, <bone_no>, <body_section>), #Sets a skeleton model bone body section (requires WSE2)
skeleton_model_clean_body_sections   = 5202 #(skeleton_model_clean_body_sections, <skeleton_model_name>), #Cleans a skeleton model's body sections (requires WSE2)
...
lhs_operations = [ # Left hand side operations make an assignment to a variable.
    agent_ai_get_behavior_target,                agent_get_attached_scene_prop,                            agent_get_item_cur_ammo,                         agent_get_team,                                                             
    agent_ai_get_cached_enemy,                   agent_get_attack_action,                                  agent_get_item_id,                               agent_get_time_elapsed_since_removed,                                                                 
    agent_ai_get_look_target,                    agent_get_class,                                          agent_get_item_slot,                             agent_get_troop_id,                                                       
    agent_ai_get_move_target,                    agent_get_combat_state,                                   agent_get_kill_count,                            agent_get_wielded_item,                                                         
    agent_ai_get_num_cached_enemies,             agent_get_crouch_mode,                                    agent_get_number_of_enemies_following,           agent_set_accuracy_modifier,                                                                   
    agent_deliver_damage_to_agent_advanced,      agent_get_defend_action,                                  agent_get_party_id,                              agent_set_damage_modifier,                                                
    agent_get_action_dir,                        agent_get_division,                                       agent_get_player_id,                             agent_set_speed_modifier,                   
    agent_get_ammo_for_slot,                     agent_get_entry_no,                                       agent_get_rider,                                 agent_set_use_speed_modifier,                                          
    agent_get_ammo,                              agent_get_group,                                          agent_get_simple_behavior,                       assign,                                                                   
    agent_get_animation,                         agent_get_horse,                                          agent_get_slot,                                                                                

    cast_ray,                                    create_game_button_overlay,                               create_mesh_overlay_with_item_id,                create_simple_text_box_overlay,                                                   
    create_button_overlay,                       create_image_button_overlay_with_tableau_material,        create_mesh_overlay_with_tableau_material,       create_slider_overlay,                                                                          
    create_check_box_overlay,                    create_image_button_overlay,                              create_mesh_overlay,                             create_text_box_overlay,                                                                          
    create_combo_button_overlay,                 create_in_game_button_overlay,                            create_number_box_overlay,                       create_text_overlay,                                                                                  
    create_combo_label_overlay,                  create_listbox_overlay,                                   create_progress_overlay,                                                                                                                                                                           

    face_keys_get_age,                           face_keys_get_hair_color,                                 face_keys_get_morph_key,                         faction_get_slot,              
    face_keys_get_beard,                         face_keys_get_hair_texture,                               face_keys_get_skin_color,                                        
    face_keys_get_face_texture,                  face_keys_get_hair,                                       faction_get_color,                                                                                                                                                                                                                               

    game_get_reduce_campaign_ai,                 get_distance_between_positions,                           get_party_ai_behavior,                           get_player_agent_own_troop_kill_count,       
    get_achievement_stat,                        get_global_cloud_amount,                                  get_party_ai_current_behavior,                   get_sq_distance_between_position_heights,        
    get_angle_between_positions,                 get_global_haze_amount,                                   get_party_ai_current_object,                     get_sq_distance_between_positions_in_meters,       
    get_average_game_difficulty,                 get_level_boundary,                                       get_party_ai_object,                             get_sq_distance_between_positions,       
    get_battle_advantage,                        get_max_players,                                          get_player_agent_kill_count,                     get_startup_ambient_light,       
    get_distance_between_positions_in_meters,    get_operation_set_version,                                get_player_agent_no,                             get_startup_ground_ambient_light,          
    get_startup_sun_light,

    item_get_abundance,                          item_get_hit_points,                                      item_get_max_ammo,                               item_get_swing_damage, 
    item_get_accuracy,                           item_get_horse_charge_damage,                             item_get_missile_speed,                          item_get_thrust_damage_type,              
    item_get_body_armor,                         item_get_horse_maneuver,                                  item_get_shield_height,                          item_get_thrust_damage,           
    item_get_difficulty,                         item_get_horse_scale,                                     item_get_slot,                                   item_get_type,
    item_get_food_quality,                       item_get_horse_speed,                                     item_get_speed_rating,                           item_get_value,         
    item_get_head_armor,                         item_get_leg_armor,                                       item_get_swing_damage_type,                      item_get_weapon_length, 
    item_get_weight,

    mission_cam_get_aperture,                    multiplayer_find_spawn_point,                             multiplayer_get_my_player,                       multiplayer_get_my_troop, 
    mission_get_time_speed,                      multiplayer_get_my_gold,                                  multiplayer_get_my_team,    

    options_get_battle_size,                     options_get_combat_ai,                                    options_get_damage_to_friends,                   overlay_get_position,
    options_get_campaign_ai,                     options_get_combat_speed,                                 options_get_damage_to_player,   
     
    party_count_companions_of_type,              party_get_num_companions,                                 player_get_gender,                               position_get_scale_x,                     
    party_count_members_of_type,                 party_get_num_prisoner_stacks,                            player_get_gold,                                 position_get_scale_y,          
    party_count_prisoners_of_type,               party_get_num_prisoners,                                  player_get_is_muted,                             position_get_scale_z,                   
    party_get_ai_initiative,                     party_get_skill_level,                                    player_get_item_id,                              position_get_x,               
    party_get_attached_party_with_rank,          party_get_slot,                                           player_get_kill_count,                           position_get_y,                 
    party_get_attached_to,                       party_get_template_id,                                    player_get_ping,                                 position_get_z,               
    party_get_battle_opponent,                   party_prisoner_stack_get_size,                            player_get_score,                                position_normalize_origin,            
    party_get_cur_town,                          party_prisoner_stack_get_troop_dna,                       player_get_slot,                                 profile_get_banner_id,                   
    party_get_current_terrain,                   party_prisoner_stack_get_troop_id,                        player_get_team_no,                              prop_instance_get_animation_target_position,                   
    party_get_free_companions_capacity,          party_stack_get_num_wounded,                              player_get_troop_id,                             prop_instance_get_current_deform_frame,                   
    party_get_free_prisoners_capacity,           party_stack_get_size,                                     player_get_unique_id,                            prop_instance_get_current_deform_progress,                
    party_get_helpfulness,                       party_stack_get_troop_dna,                                player_get_value_of_original_items,              prop_instance_get_position,                                
    party_get_icon,                              party_stack_get_troop_id,                                 position_get_distance_to_ground_level,           prop_instance_get_scale,                                
    party_get_ignore_with_player_party,          party_template_get_slot,                                  position_get_distance_to_terrain,                prop_instance_get_scene_prop_kind,                                      
    party_get_morale,                            player_get_agent_id,                                      position_get_rotation_around_x,                  prop_instance_get_starting_position,                           
    party_get_num_attached_parties,              player_get_banner_id,                                     position_get_rotation_around_y,                  prop_instance_get_variation_id_2,                              
    party_get_num_companion_stacks,              player_get_death_count,                                   position_get_rotation_around_z,                  prop_instance_get_variation_id,                                    
    prop_instance_is_animating,

    quest_get_slot,

    scene_get_slot,                                    store_and,                                          store_mission_timer_b_msec,                      store_random_party_in_range,        
    scene_item_get_instance,                           store_asin,                                         store_mission_timer_b,                           store_random_party_of_template,     
    scene_item_get_num_instances,                      store_atan,                                         store_mission_timer_c_msec,                      store_random_quest_in_range,      
    scene_prop_get_hit_points,                         store_atan2,                                        store_mission_timer_c,                           store_random_troop_to_capture,      
    scene_prop_get_instance,                           store_attacker_count,                               store_mod,                                       store_random_troop_to_raise,          
    scene_prop_get_max_hit_points,                     store_attribute_level,                              store_mul,                                       store_random,         
    scene_prop_get_num_instances,                      store_character_level,                              store_normalized_team_count,                     store_relation,          
    scene_prop_get_slot,                               store_conversation_agent,                           store_num_free_prisoner_stacks,                  store_remaining_team_no,     
    scene_prop_get_team,                               store_conversation_troop,                           store_num_free_stacks,                           store_repeat_object,       
    scene_prop_get_visibility,                         store_cos,                                          store_num_parties_created,                       store_script_param_1,         
    scene_spawned_item_get_instance,                   store_current_day,                                  store_num_parties_destroyed_by_player,           store_script_param_2,       
    scene_spawned_item_get_num_instances,              store_current_hours,                                store_num_parties_destroyed,                     store_script_param,             
    server_get_add_to_game_servers_list,               store_current_scene,                                store_num_parties_of_template,                   store_sin,            
    server_get_anti_cheat,                             store_defender_count,                               store_num_regular_prisoners,                     store_skill_level,             
    server_get_changing_game_type_allowed,             store_distance_to_party_from_party,                 store_or,                                        store_sqrt,           
    server_get_combat_speed,                           store_div,                                          store_partner_faction,                           store_sub,          
    server_get_control_block_dir,                      store_encountered_party,                            store_partner_quest,                             store_tan,          
    server_get_friendly_fire_damage_friend_ratio,      store_encountered_party2,                           store_party_size_wo_prisoners,                   store_time_of_day,      
    server_get_friendly_fire_damage_self_ratio,        store_enemy_count,                                  store_party_size,                                store_trigger_param_1,         
    server_get_friendly_fire,                          store_faction_of_party,                             store_pow,                                       store_trigger_param_2,          
    server_get_ghost_mode,                             store_free_inventory_capacity,                      store_proficiency_level,                         store_trigger_param_3,          
    server_get_max_num_players,                        store_friend_count,                                 store_quest_item,                                store_trigger_param,            
    server_get_melee_friendly_fire,                    store_item_kind_count,                              store_quest_number,                              store_troop_count_companions,        
    server_get_renaming_server_allowed,                store_item_value,                                   store_quest_troop,                               store_troop_count_prisoners,          
    store_acos,                                        store_last_sound_channel,                           store_random_armor,                              store_troop_faction,           
    store_add,                                         store_main_party_wounded,                           store_random_equipment,                          store_troop_gold,       
    store_agent_hit_points,                            store_mission_timer_a_msec,                         store_random_horse,                              store_troop_health,              
    store_ally_count,                                  store_mission_timer_a,                              store_random_in_range,                           store_troop_kind_count,                     
    store_troop_value,                                 store_zoom_amount,                                  str_store_agent_face_keys,

    team_get_bot_death_count,                         team_get_movement_order,                             troop_get_inventory_slot_modifier,               troop_inventory_slot_get_item_max_amount,    
    team_get_bot_kill_count,                          team_get_riding_order,                               troop_get_inventory_slot,                        try_for_agents,  
    team_get_faction,                                 team_get_score,                                      troop_get_slot,                                  try_for_parties,    
    team_get_gap_distance,                            team_get_slot,                                       troop_get_type,                                  try_for_players,      
    team_get_hold_fire_order,                         team_get_weapon_usage_order,                         troop_get_upgrade_troop,                         try_for_prop_instances,   
    team_get_kill_count,                              troop_get_class,                                     troop_get_xp,                                    try_for_range,  
    team_get_leader,                                  troop_get_inventory_capacity,                        troop_inventory_slot_get_item_amount,            try_for_range_backwards,   
]
# add_missile, MOTO causes local variable unused warnings? 

global_lhs_operations = [
    val_lshift,       val_add,          val_mul,           val_max,          val_mod,
    val_rshift,       val_sub,          val_div,           val_min,  
]

can_fail_operations = [
    agent_check_offer_from_agent,     agent_is_alive,          agent_is_in_line_of_sight,            agent_is_routed,                                          
    ...
    cast_ray,                         check_quest_concluded,   check_quest_finished,               class_is_listening_order,                             
    ...
    hero_can_join,                    hero_can_join_as_prisoner,
    ...
    scene_allows_mounted_units,                   scene_prop_slot_ge,      
...
lhs_operations += [
  player_get_anticheat_stat,
  server_get_anticheat_option,
  edit_mode_get_mode,
]
